...
export const saveCompletedTopics = async (userId: string, completedTopics: Record<string, boolean>) => {
  const { error } = await supabase
    .from('user_completed_topics')
    .upsert(
      Object.entries(completedTopics).map(([topicId, completed]) => ({
        user_id: userId,
        topic_id: topicId,
...
export const loadCompletedTopics = async (userId: string): Promise<Record<string, boolean>> => {
  ...
  return data.reduce((acc: Record<string, boolean>, topic) => {
    acc[topic.topic_id] = !!topic.completed_at;
    return acc;
  }, {});
};
